...
<<<<<<< HEAD
ColorPointCloud::ColorPointCloud(const rclcpp::NodeOptions &options)
    : Node("color_point_cloud_node", options),
      transform_provider_ptr_(
          std::make_shared<TransformProvider>(this->get_clock())) {
    ...
    ColorPointCloud::ColorPointCloud(const rclcpp::NodeOptions &options) : Node("color_point_cloud_node", options),
                                                                           transform_provider_ptr_(
                                                                                   std::make_shared<TransformProvider>(
                                                                                           this->get_clock())) {
        RCLCPP_INFO(this->get_logger(), "ColorPointCloud node started");
>>>>>>> 9115dfa (Initial commit with my changes)
  ...
  // Declare and get parameters
  {
    ...
    point_cloud_frame_id_ =
        this->get_parameter("point_cloud_frame_id").as_string();

    this->declare_parameter<int>("image_type", 0);
    image_type_ =
        static_cast<ImageType>(this->get_parameter("image_type").as_int());

    this->declare_parameter<std::string>("image_topic_last_name", "/image_raw");
    image_topic_last_name_ =
        this->get_parameter("image_topic_last_name").as_string();

    this->declare_parameter<std::string>("camera_info_topic_last_name",
                                         "/camera_info");
    camera_info_topic_last_name_ =
        this->get_parameter("camera_info_topic_last_name").as_string();

    // camera_topics
    this->declare_parameter<std::vector<std::string>>(
        "camera_topics", std::vector<std::string>());
  ...
  // Create camera object, create subscriber to image and camera_info
  {
    for (const auto &camera_topic : camera_topics_) {
      ...
      std::string camera_info_topic =
          camera_topic + camera_info_topic_last_name_;

      CameraTypePtr camera_type_ptr =
          std::make_shared<CameraType>(image_topic, camera_info_topic);
      camera_type_stdmap_[camera_topic] = camera_type_ptr;

      this->image_subscribers_.push_back(
          this->create_subscription<sensor_msgs::msg::Image>(
              image_topic, rclcpp::SystemDefaultsQoS(),
              [this, image_topic, camera_topic](
                  const sensor_msgs::msg::Image::ConstSharedPtr &msg) {
                // RCLCPP_INFO(this->get_logger(), "Received image on topic %s",
                // camera_topic.c_str());
      ...
      this->camera_info_subscribers_.push_back(
          this->create_subscription<sensor_msgs::msg::CameraInfo>(
              camera_info_topic, rclcpp::SystemDefaultsQoS(),
              [this, camera_info_topic, camera_topic](
                  const sensor_msgs::msg::CameraInfo::ConstSharedPtr &msg) {
                // RCLCPP_INFO(this->get_logger(), "Received camera_info on
                // topic %s", camera_topic.c_str());
...

  {
    point_cloud_subscriber_ =
        this->create_subscription<sensor_msgs::msg::PointCloud2>(
            point_cloud_topic_, rclcpp::SystemDefaultsQoS(),
            std::bind(&ColorPointCloud::point_cloud_callback, this,
                      std::placeholders::_1));

    point_cloud_publisher_ =
        this->create_publisher<sensor_msgs::msg::PointCloud2>(
            "/points_color", rclcpp::SystemDefaultsQoS());
  }
  ...
  // Set timer
  {
    ...
    timer_ =
        rclcpp::create_timer(this, this->get_clock(), period_ns,
...
void ColorPointCloud::timer_callback() {
  for (const auto &pair : camera_type_stdmap_) {
    if (pair.second->get_image_msg() == nullptr ||
      ...
      //                RCLCPP_INFO(this->get_logger(), "Image or camera info is
      //                null for topic: %s", pair.first.c_str());
      continue;
    }
...
void ColorPointCloud::point_cloud_callback(
    const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg) {

  sensor_msgs::msg::PointCloud2 cloud_color_msg;
  std::for_each(
      camera_type_stdmap_.begin(), camera_type_stdmap_.end(),
      [this, &cloud_color_msg,
       msg](std::pair<std::string, CameraTypePtr> pair) {
        if (pair.second->get_image_msg() == nullptr ||
            pair.second->get_camera_info() == nullptr ||
            !pair.second->is_info_initialized() ||
        ...
        for (size_t i = 0; i < cloud.getPointCount(); ++i) {
          ...
          const Eigen::Vector4d pt_camera = T_lidar_camera.inverse() * point4d;
          const Eigen::Vector2i pt_2i = pair.second->project_lidar_to_camera(pt_camera.head<3>()).cast<int>();
          ...
          if (x < 0 || x > pair.second->get_image_width() || y < 0 ||
            ...
            iter_y[0] = point.y;
            iter_z[0] = point.z;
            // iter_ring[0] = std::numeric_limits<uint16_t>::quiet_NaN(); //point.ring;
            // iter_intensity[0] = std::numeric_limits<float>::quiet_NaN(); //point.intensity;
...
<<<<<<< HEAD
=======
>>>>>>> 7345d07 (Initial commit with my changes)
>>>>>>> 9115dfa (Initial commit with my changes)
